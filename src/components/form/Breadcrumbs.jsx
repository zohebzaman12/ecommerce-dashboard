...
const Breadcrumbs = () => {
  ...
  const currentIndex = paths.findIndex(crumb => location.pathname === crumb.path);
        ...
        <span key={index} className="flex items-center">
          <span
            className={`px-3 py-1 text-sm rounded-md font-medium ${
              index <= currentIndex ? 'bg-customBlueLight text-customBlue' : 'text-customGrayDark'
            }`}
          >
            {crumb.label}
          </span>
          {index < paths.length - 1 && (
            <span className="text-customGrayDark text-lg ml-2 font-medium"> &gt; </span>
          )}
        </span>
      ))}
    </nav>
